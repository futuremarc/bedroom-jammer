...
var creaturePlayers = [];
creaturePlayers.push( new Tone.Player("./audio/metalsound.mp3") );
...
// 
  ...
  for (var i = 0; i < cloudPlayers.length; i++) {
   ...
   for (var i = 0; i < creaturePlayers.length; i++) {
    // leafPlayers[i].toMaster();
    creaturePlayers[i].connect(masterMix);
    creaturePlayers[i].loop = true;
    creaturePlayers[i].setLoopPoints(loopStart, loopEnd)
    creaturePlayers[i].start(0);
    creaturePlayers[i].volume.value = -64;
  }
...
var stopLooping = function(aPlayer) {
  ...
  var oneMeasure = Tone.Transport.transportTimeToSeconds("4:0:0");
...
// osc.toMaster();
// envFollower.connect(osc.setVolume);
...
playStar = function() {
  ...
  var note = whatKey[numb % whatKey.length];
  var octave = String( Math.floor(numb / 5.1) + 3);
